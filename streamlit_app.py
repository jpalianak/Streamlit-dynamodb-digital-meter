...
with row1_col1:
    ...
    st.markdown(f'<h4 style="font-size: 20px;"><strong>Máximo valor CNN:</strong> <em>{max_event} Amp</em></h4>', unsafe_allow_html=True)

with row1_col2:
    fecha_event = df_cnn_filtered.loc[df_cnn_filtered['Value'].idxmax(), 'Date_num']
    fecha_event = pd.to_datetime(fecha_event * 10**9)
    st.markdown(f'<h4 style="font-size: 20px;"><strong>Fecha máximo valor CNN:</strong> <em>{fecha_event}</em></h4>', unsafe_allow_html=True)

with row1_col3:
    mean_event = round(df_cnn_filtered['Value'].mean() * factor, 2)
    st.markdown(f'<h4 style="font-size: 20px;"><strong>Valor promedio CNN:</strong> <em>{mean_event} Amp</em></h4>', unsafe_allow_html=True)
...
with row2_col1:
    ...
    st.markdown(f'<h4 style="font-size: 20px;"><strong>Máximo valor OpenCV:</strong> <em>{max_event_opencv} Amp</em></h4>', unsafe_allow_html=True)
...
with row2_col2:
    ...
    st.markdown(f'<h4 style="font-size: 20px;"><strong>Fecha máximo valor OpenCV:</strong> <em>{fecha_event_opencv}</em></h4>', unsafe_allow_html=True)

with row2_col3:
    mean_event_opencv = round(df_opencv_filtered['Value'].mean() * factor, 2)
    st.markdown(f'<h4 style="font-size: 20px;"><strong>Valor promedio OpenCV:</strong> <em>{mean_event_opencv} Amp</em></h4>', unsafe_allow_html=True)
